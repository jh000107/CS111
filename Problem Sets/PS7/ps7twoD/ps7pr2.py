...
def inner_grid(height, width):
    """ creates and returns a 2-D list of height rows and columns in which
...
def copy(grid):
    """ creates and returns a deep copy of grid-a newm separate 2-D list that
        has the same dimensions and cell values as grid
        input: grid --> a 2-D list
    """
    copied_grid = create_grid(len(grid), len(grid[0]))
    for r in range(len(grid)):
        for c in range(len(grid[0])):
            copied_grid[r][c] = grid[r][c]
    return copied_grid
